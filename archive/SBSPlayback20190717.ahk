...
title =  sMXF Player v2.2 by sendust 20190717
osc_reader := "C:\ahk\BMD_PLAYER_v2\read_osc_caspar_v22.ahk"
...
Gui, add, button, xp+120 yp w60 h30 hwndhsetin gsendin, IN
Gui, add, button, xp+70 yp w60 h30 hwndhsetout gsendout, OUT
...
Gui, add, Text, xm+370 ym w400 h30 hwndhplist_duration Center, PLAYLIST DURATION    00:00:00.000
...
if StrLen(xmlfile) > 20
{
    ...
    */	
	...
	if (caspar.listplay and (caspar.pbindex = LV_GetCount()) and (chk_loop = "PLAYLIST"))		; PLAYLSIT LOOP CASE, Last clip playing, load first clip
		caspar.pbindex_next := 1
	...
	if (caspar.listplay and (caspar.pbindex < LV_GetCount()))
	...
	if (caspar.listplay and (caspar.timerem < 6) and (caspar.timerem > 4) and !caspar.loadbg_try and (caspar.pbposition <= LV_GetCount()))			; current play is listplay and remaining time is between 6 and 4
	{
...
if ((caspar.pbindex = LV_GetCount()) and (caspar.timerem < 0.1) and caspar.listplay)		; pbindex reach LV count and no remaing frame -- Finish listplay
{
	caspar.listplay := 0
...
loadlvclip(byref caspar, byref lvdata)
{
	...
	text_to_send := "loadbg 1-1 """ . lvdata.clip . """ SEEK " . caspar.in . " " . caspar.duration .  " MIX 10 auto`r`n"	
...
get_plist_duration()
{
	...
	show_duration := "PLAYLIST DURATION " . secondtotc(mylist_new[1])
