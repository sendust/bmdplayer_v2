...
title =  sMXF Player Remote by sendust 20191015
...
while(1)		; Read serial port data
{
	if chk_serial
	{
		...
		if byte_received
		{
			...
			if ((byte_received = 2) and (InStr(cmd_list, data)))	; read 2 byte and there is valid command data
				{	
					count_data_recv += 1
					consoleout(data . " ------ " . cmd_recv[data] . " ------- "  .  count_data_recv)
					SetTimer, % keypad_to_goto[cmd_recv[data]], -1
				}
		}
	}
