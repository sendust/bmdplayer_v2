/*		
	...
	2020/6/9 Enhancd log routine (monitor log size before writing log)
...
title =  sMXF Player v2.2 by sendust 20200609
...
Gui, add, ListView, xm+370 ym+30 w400 h410 hwndhlistview vVLV glvclick NoSortHdr NoSort ReadOnly, STATUS|TITLE|IN|OUT|DURATION|CLIP LIST
...
IniRead, temp, splay.ini, caspar, character_out
caspar.character_out := temp
IniRead, temp, splay.ini, caspar, port_tc
caspar.port_tc := temp
IniRead, temp, splay.ini, caspar, html_tc
caspar.html_tc := temp
IniRead, temp, splay.ini, caspar, grid_out
...
if caspar.character_out
{
	try
	{
		;casparamcp.sendText("load " . caspar.character_out . "-1 route://1-1`r`n")				; duplicate character out channel with master channel
		;Sleep, 150
		casparamcp.sendText("play " . caspar.character_out . "-1 route://1-1`r`n")				; synchronize play with master channel
		Sleep, 150
		casparamcp.sendText("load " . caspar.character_out . "-2 [html] " . caspar.html_tc . "`r`n")				; load html tc
		Sleep, 150
...
get_remote_address()
{
	IniRead, address, splay.ini, remote, address, 127.0.0.1
	IniRead, port, splay.ini, remote, port, 8989
...
getfile_plist(list)
{
	return A_WorkingDir . "\PLIST\" . list . ".txt"
}
...
try
{	
	casparamcp.sendText(cmd_load . " 1-1 """ . caspar.medianame . """ SEEK " . caspar.in . " " . caspar.duration . " " . caspar.loop . "`r`n")
}
catch, err
	updatelog(err.Message)
	
updatelog(cmd_load . " 1-1 """ . caspar.medianame . """ SEEK " . caspar.in . " " . caspar.duration . " " . caspar.loop )
...
{	
	casparamcp.sendText("play 1-1`r`n")
	;Sleep, 100											; disabled 2020/4/1
	;casparamcp.sendText("info 1-1`r`n")						; disabled 2020/4/1
	;casparamcp.sendText("CALL 1-1 " . caspar.loop . "`r`n")						; disabled 2020/4/1
}
catch, err
	updatelog(err.Message)
...
;	casparamcp.sendText("info 1-1`r`n")			; disabled 2020/4/1
	...
	updatelog(err.Message)
updatelog("Play command 1-1")			; disabled 2020/4/1
...
checklogfile(chkfile)
{
	...
	if (size > 3000000)
	{
		updatelog("Log file size is " . size . " Bytes  ///////     Backup old log file ................")
...
secondtotc_drop_old_backup(sec)
{
	sec := format("{:10.3f}", sec)
	sec_out := Floor(sec)
