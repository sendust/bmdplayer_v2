/*		
	...
	2020/11/30  introduce simple_tc variable
...
title := "sendust multi channel player - " . get_casparchannel(A_ScriptName) . " -  2021/1/5"
finishday :=  20251231235959
...
debug_mode := 0
simple_tc := 1					; simple_tc ; simplified TC display (for SBS ts-6)			; 2020/11/30
...
Gui, add, ListView, xm+370 ym+30 w800 h410 hwndhlistview vVLV glvclick NoSortHdr NoSort ReadOnly -LV0x10, STATUS|TITLE|IN|OUT|DURATION|CLIP LIST|AUDIO
...
IniRead, tts,  % getinifile(A_ScriptFullPath), caspar, tts, 0

IniRead, temp,  % getinifile(A_ScriptFullPath), remote, force_on, 0			; read auto remote on
if (temp)
{
	GuiControl,, %hchk_remote%, 1
	SetTimer, remote, -3000
	updatelog("Remote Enabled from INI")
}
...
watchdog.count := 4					; invalid response count limit
...
resources.push(A_ScriptDir . "\caspa_info.py")					; new  2020/1/5
...
if !caspar.remote_client			; Check if remote client mode
{
	...
	Run, %osc_reader%				; run osc only local client mode
}
...
initcaspar_stage2:
addressamcp := [caspar.address_caspar, "5250"]
...
if ((A_GuiEvent = "DoubleClick") or (A_GuiEvent = "D"))
{
	if !caspar.listplay					; During listplaying, pbindex cannot be modified manually
	{
		...
		;caspar.preindex := LV_GetNext(0)			; deprecated 2020/11/9
...
caspartowinpath(byref caspar)
{
	...
	FileAppend, `r`n------Caspar path to OSpath---------`r`nfullpath =  %tempfile% , *
	Loop, Files, %path%\*.*, R
	{
...
FileAppend, % "`nmedia_fullpath = " . media.fullpath , *
...
if (A_GuiControl = "LOAD |◁`r`nPREROLL")				; Right Click LOAD Button
{
	...
	try
	{
		casparamcp.sendText("clear " . caspar.channel_layer  . "`r`n")				; Unload main output video layer
		if (caspar.character_out )			
		{
			casparamcp.sendText("clear " . caspar.character_out . "-1`r`n")			; Unload character output video layer
			casparamcp.sendText("play " . caspar.character_out . "-1 route://" . caspar.chindex . "-1`r`n")			; reload character output channel (from pgm channel)
			casparamcp.sendText("play " . caspar.character_out . "-2 [html] " . caspar.html_tc . "`r`n")				; load html tc
		}
	}
	catch, err
...
decode_python_info(text)
{
	...
	info_caspar.time1_clip := rdata["clip0"]
	info_caspar.time2_clip := rdata["clip1"]
	info_caspar.time1_foreground := rdata["time0"]
	info_caspar.time2_foreground := rdata["time1"]
	info_caspar.time_rem := rdata["time_rem"]
	info_caspar.time_rem := rdata["time_run"]
...
{	
	...
	binary_run := "python.exe " . """" . A_WorkingDir . "\caspa_info.py" . """"
