...
def drawframe():
    ...
    text = font.render("time0 - " + str(info_display.time[0]), True, (255, 255, 255))
    window.screen.blit(text, info_display.position["time0"])
    text = font.render("time1 - " + str(info_display.time[1]), True, (255, 255, 255))
    window.screen.blit(text, info_display.position["time1"])        
    
    text = font.render("clip0 - " + str(info_display.clip[0]), True, (255, 255, 255))
    window.screen.blit(text, info_display.position["clip0"])    
    text = font.render("clip1 - " + str(info_display.clip[1]), True, (255, 255, 255))
    window.screen.blit(text, info_display.position["clip1"])    
    


    font = pygame.font.SysFont("gulimgulimchedotumdotumche", 20)
    text = font.render("path = " + str(info_display.path_foreground[0]), True, (255, 255, 255))
    window.screen.blit(text, info_display.position["path_foreground"])    
    
    text = font.render("path = " + str(info_display.name_background[0]), True, (255, 255, 255))
    window.screen.blit(text, info_display.position["name_background"])    
    
    text = font.render("paused = " + str(info_display.paused[0]), True, (255, 255, 255))
    window.screen.blit(text, info_display.position["paused"])   

    text = font.render("loop = " + str(info_display.loop[0]), True, (255, 255, 255))
    window.screen.blit(text, info_display.position["loop"])   

    text = font.render("producer = " + str(info_display.producer[0]), True, (255, 255, 255))
    window.screen.blit(text, info_display.position["producer"])   
...
class LEVELMETER:
    ...
    position_start = 30
    position_end = 380
    clearance = ( position_end - position_start ) / 7
    position = []
    for i in range(0, 8):
        position.append((position_start + i * clearance, 200))
    width = 26
    color = (255, 255, 0)
...
class GAMEWINDOW():
    def __init__(self, size_x, size_y):
        ...
        pygame.init()
        self.title = "Caspar v2.3 OSC Monitor by sendust"
