...
starty := 10
title := "Caspar v2.2 OSC Monitor by sendust 20200305"
...
osc_field.message[11] := "/channel/2/stage/layer/1/foreground/file/time"
osc_field.len[11] := StrLen(osc_field.message[11])

osc_field.message[12] := "/channel/2/mixer/audio/volume"
osc_field.len[12] := StrLen(osc_field.message[12])
...
Gui, add, edit, xm w400 h20 hwndhtext11 ReadOnly Center, info
Gui, add, edit, w400 h30 hwndhtext12 ReadOnly Center, info2
Gui, add, Text, xm yp+40, CH1
Gui, add, Text, xp+40 yp w300 hwndhaudio11, --
Gui, add, Text, xm yp+15, CH2
Gui, add, Text, xp+40 yp w300 hwndhaudio12, --
Gui, add, Text, xm yp+15, CH3
Gui, add, Text, xp+40 yp w300 hwndhaudio13, --
Gui, add, Text, xm yp+15, CH4
Gui, add, Text, xp+40 yp w300 hwndhaudio14, --
Gui, add, Text, xm yp+15, CH5
Gui, add, Text, xp+40 yp w300 hwndhaudio15, --
Gui, add, Text, xm yp+15, CH6
Gui, add, Text, xp+40 yp w300 hwndhaudio16, --
Gui, add, Text, xm yp+15, CH7
Gui, add, Text, xp+40 yp w300 hwndhaudio17, --
Gui, add, Text, xm yp+15, CH8
Gui, add, Text, xp+40 yp w300 hwndhaudio18, --
...
onUDPRecv(this)
{
	...
	global  htext11, htext12
	global haudio1, haudio2, haudio3, haudio4, haudio5, haudio6, haudio7, haudio8
	global haudio11, haudio12, haudio13, haudio14, haudio15, haudio16, haudio17, haudio18
	buffer := ""
	temp := Object()
	offset_found := 0
	message := ""
	...
	Loop, % (length - osc_field.len[11]) / 4
	{
		message :=  StrGet(&buffer + A_Index * 4, osc_field.len[12], "cp437") 			; Human readable message
		;FileAppend, %message%`r`n, *														; For Capture osc message

		if (InStr(message, osc_field.message[12]))													; check if there is Audio level meter data
		{
			offset_found := A_Index* 4

			temp1 := NumGet(&buffer, offset_found + osc_field.len[12] + 14, "UChar") * 0x1000000 + NumGet(&buffer, offset_found + osc_field.len[12] + 15, "UChar") * 0x10000 + NumGet(&buffer, offset_found + osc_field.len[12] + 16, "UChar") * 0x100 + NumGet(&buffer, offset_found + osc_field.len[12] + 17, "UChar")
			temp2 := NumGet(&buffer, offset_found + osc_field.len[12] + 18, "UChar") * 0x1000000 + NumGet(&buffer, offset_found + osc_field.len[12] + 19, "UChar") * 0x10000 + NumGet(&buffer, offset_found + osc_field.len[12] + 20, "UChar") * 0x100 + NumGet(&buffer, offset_found + osc_field.len[12] + 21, "UChar")
			temp3 := NumGet(&buffer, offset_found + osc_field.len[12] + 22, "UChar") * 0x1000000 + NumGet(&buffer, offset_found + osc_field.len[12] + 23, "UChar") * 0x10000 + NumGet(&buffer, offset_found + osc_field.len[12] + 24, "UChar") * 0x100 + NumGet(&buffer, offset_found + osc_field.len[12] + 25, "UChar")
			temp4 := NumGet(&buffer, offset_found + osc_field.len[12] + 26, "UChar") * 0x1000000 + NumGet(&buffer, offset_found + osc_field.len[12] + 27, "UChar") * 0x10000 + NumGet(&buffer, offset_found + osc_field.len[12] + 28, "UChar") * 0x100 + NumGet(&buffer, offset_found + osc_field.len[12] + 29, "UChar")
			temp5 := NumGet(&buffer, offset_found + osc_field.len[12] + 30, "UChar") * 0x1000000 + NumGet(&buffer, offset_found + osc_field.len[12] + 31, "UChar") * 0x10000 + NumGet(&buffer, offset_found + osc_field.len[12] + 32, "UChar") * 0x100 + NumGet(&buffer, offset_found + osc_field.len[12] + 33, "UChar")
			temp6 := NumGet(&buffer, offset_found + osc_field.len[12] + 34, "UChar") * 0x1000000 + NumGet(&buffer, offset_found + osc_field.len[12] + 35, "UChar") * 0x10000 + NumGet(&buffer, offset_found + osc_field.len[12] + 36, "UChar") * 0x100 + NumGet(&buffer, offset_found + osc_field.len[12] + 37, "UChar")
			temp7 := NumGet(&buffer, offset_found + osc_field.len[12] + 38, "UChar") * 0x1000000 + NumGet(&buffer, offset_found + osc_field.len[12] + 39, "UChar") * 0x10000 + NumGet(&buffer, offset_found + osc_field.len[12] + 40, "UChar") * 0x100 + NumGet(&buffer, offset_found + osc_field.len[12] + 41, "UChar")
			temp8 := NumGet(&buffer, offset_found + osc_field.len[12] + 42, "UChar") * 0x1000000 + NumGet(&buffer, offset_found + osc_field.len[12] + 43, "UChar") * 0x10000 + NumGet(&buffer, offset_found + osc_field.len[12] + 44, "UChar") * 0x100 + NumGet(&buffer, offset_found + osc_field.len[12] + 45, "UChar")
			...
			GuiControl,, %haudio11%, %temp1%
			GuiControl,, %haudio12%, %temp2%
			GuiControl,, %haudio13%, %temp3%
			GuiControl,, %haudio14%, %temp4%
			GuiControl,, %haudio15%, %temp5%
			GuiControl,, %haudio16%, %temp6%
			GuiControl,, %haudio17%, %temp7%
			GuiControl,, %haudio18%, %temp8%
		}
		
		message :=  StrGet(&buffer + A_Index * 4, osc_field.len[11], "cp437") 			; Human readable message
		if (InStr(message, osc_field.message[11]))													; check if there is interest data field
		{
			offset_found := A_Index * 4
			for key, val in array_16bit
				temp[key] := NumGet(&buffer, offset_found + osc_field.len[11] + val, "UChar")
			GuiControl,, %htext11%,  %offset_found%/%length% - %message%
			;time_pos := format("{:10.3f}", IEEE754(temp[8], temp[9], temp[10], temp[11]))
			;time_duration := format("{:10.3f}", IEEE754(temp[12], temp[13], temp[14], temp[15]))
			;time_pos := secondtotc_drop(IEEE754(temp[8], temp[9], temp[10], temp[11]))
			time_pos := secondtotc_drop(HexToFloat(temp[8] * 0x1000000 + temp[9] * 0x10000 + temp[10] * 0x100 + temp[11]))				; Get 4 byte Hex number
			time_duration := secondtotc_drop(HexToFloat(temp[12] * 0x1000000 +  temp[13] * 0x10000 + temp[14] * 0x100 + temp[15]))			; Get 4 byte Hex number
			;show_debuginfo(temp)
			GuiControl,, %htext12%,  %time_pos% / %time_duration%
		}
	}
}
