#
#   Caspar channel infomation reader by sendust
#   Lastedit : 2020/12/15
#    
...
class Caspar_info:
    ...
    def __init__(self):
        ...
        self.time_rem = float(max_duration)
        self.time_run = 0
        self.smpte_rem = ""
        self.smpte_run = ""
        self.smpte_dur = ""
    ...
    def get_time_rem(self):
        ...
        time_rem = float(self.clip0) + float(self.clip1) - float(self.time0)
        time_run = float(self.time0) - float(self.clip0)
        if time_rem < 0.0001:
            time_rem = 0
        if time_run < 0:
            time_run = 0
        ...
        try:
            if float(self.clip1) > 0:
                self.position_run = float(time_run) / float(self.clip1)
                self.smpte_dur = frames_to_timecode(float(self.clip1) * 29.97)
            else:
                self.position_run = 0
                self.smpte_dur = frames_to_timecode(float(self.max_duration) * 29.97)
        except Exception as e:
            updatelog("Error calculating position in percent" + str(e))
            self.position_run = 0
        self.smpte_rem = frames_to_timecode(time_rem * 29.97)
        self.smpte_run = frames_to_timecode(time_run * 29.97)
        self.time_tick = time.perf_counter()
        return time_rem
...
def analysis_info(xml):
    ...
    try:
        ...
        node = root.findall('.//framerate')
        if node:
            caspar_info.fps0_channel = checkif_nodeis_null(node[0], 0)
            caspar_info.fps1_channel = checkif_nodeis_null(node[1], 0)
        else:
            caspar_info.fps0_channel = -1
            caspar_info.fps1_channel = -1
        
        node = root.findall('.//stage/layer/layer_1/foreground/file/streams/file/streams_0/fps')
        if node:
            caspar_info.fps0_stream = checkif_nodeis_null(node[0], 0)
            caspar_info.fps1_stream = checkif_nodeis_null(node[1], 0)
        else:
            caspar_info.fps0_stream = -1
            caspar_info.fps1_stream = -1
            
        node = root.findall('.//stage/layer/layer_1/foreground/file/clip')
        if node:
            caspar_info.clip0 = checkif_nodeis_null(node[0], 0)
            caspar_info.clip1 = checkif_nodeis_null(node[1], 0)
        else:
            caspar_info.clip0 = 0
            caspar_info.clip1 = caspar_info.max_duration

        node = root.findall('.//mixer/audio/volume')
        if node:
            caspar_info.volume0 = checkif_nodeis_null(node[0], 0)
        else:
            caspar_info.volume0 = 9999

        node = root.find('.//stage/layer/layer_1/foreground/file/name')
        caspar_info.name_foreground = checkif_nodeis_null(node, "")
        
        node = root.find('.//stage/layer/layer_1/foreground/file/path')
        caspar_info.path_foreground = checkif_nodeis_null(node, "")
        if caspar_info.path_foreground:
            caspar_info.path_foreground = caspar_info.path_foreground.replace("/", "\\")
            caspar_info.path_foreground = caspar_info.path_foreground.replace("\\\\\\", "\\")
        
        node = root.findall('.//stage/layer/layer_1/foreground/file/time')
        if node:
            caspar_info.time0 = checkif_nodeis_null(node[0], 0)
            caspar_info.time1 = checkif_nodeis_null(node[1], 0)
        ...
        node =  root.find('.//stage/layer/layer_1/foreground/loop')
        caspar_info.loop = checkif_nodeis_null(node, "")
        
        node = root.find('.//stage/layer/layer_1/foreground/paused')
        caspar_info.paused = checkif_nodeis_null(node, "")

        node =  root.find('.//stage/layer/layer_1/foreground/producer')
        caspar_info.producer_foreground = checkif_nodeis_null(node, "")

        node = root.find('.//stage/layer/layer_1/background/file/name')
        caspar_info.name_background = checkif_nodeis_null(node, "")
...
class Amcp:
    ...
    def sendinfo(self, channel):
        try:
            print("Send info command to caspar")
